...
FIREBASE_AUTH= "BaP7SSMKkNLJkVFFcXpqYPJOpKQBWFnNknESzsoH" # KHÓA BẢO MẬT ĐỂ GHI DỮ LIỆU
...
def get_owm_current():
    try:
        r = requests.get(f"http://api.openweathermap.org/data/2.5/air_pollution?lat={LAT}&lon={LON}&appid={OWM_KEY}", timeout=5)
...
def get_daily_pm25_history():
    ...
    for days_ago in range(3, 0, -1):
        ds = (datetime.utcnow()+timedelta(hours=7)-timedelta(days=days_ago)).strftime("%Y-%m-%d")
        try:
...
def get_aqicn_data():
    try:
        r = requests.get(f"https://api.waqi.info/feed/hanoi/?token={AQICN_TOKEN}", timeout=8)
        if r.status_code == 200:
            d = r.json()
            if d['status'] == 'ok':
...
def get_owm_weather_forecast():
    try:
        r = requests.get(f"https://api.openweathermap.org/data/2.5/forecast?lat={LAT}&lon={LON}&appid={OWM_KEY}&units=metric", timeout=5)
        if r.status_code == 200:
            by_date = {}
            ...
            for dk, entries in by_date.items():
                ...
                mid    = next((e for e in entries if '11' <= (datetime.utcfromtimestamp(e['dt'])+timedelta(hours=7)).strftime('%H') < '14'), entries[len(entries)//2])
                result[dk] = {'temp_max': round(max(temps)), 'temp_min': round(min(temps)), 'humidity': round(sum([e['main']['humidity'] for e in entries])/len(entries)), 'emoji': w_emoji(mid['weather'][0]['icon'])}
            return result
    except: pass
    return {}
...
def build_7day_forecast():
    ...
    for i in range(7):
        ...
        if i == 0 and current_aqi: aqi = current_aqi
        elif d_str in aqicn_map: aqi = pm25_to_aqi(aqicn_map[d_str])
        else: aqi = 0
    ...
    return {
        'updated': (datetime.utcnow()+timedelta(hours=7)).strftime('%d/%m/%Y %H:%M'),
        'days': days,
    }
...
def get_or_refresh_forecast():
    ...
    if _forecast_cache is None or (now - _forecast_time) > 3600:
        print("[FORECAST] Làm mới dự báo 7 ngày...")
        _forecast_cache = build_7day_forecast()
        _forecast_time  = now
        try:
            # Ghi lên Firebase bắt buộc có auth
            requests.put(f"{FIREBASE_URL}/weather_7day.json?auth={FIREBASE_AUTH}", json=_forecast_cache, timeout=5)
            print("[FIREBASE] Đã ghi weather_7day thành công!")
        except Exception as e:
            print(f"[WARN] Lỗi ghi Firebase weather: {e}")
...
def build_features(pm2_5, owm, history, target_date):
    ...
    return {
        ...
        'carbon_monoxide':owm.get('co',HANOI_AVG['carbon_monoxide']),'nitrogen_dioxide':owm.get('no2',HANOI_AVG['nitrogen_dioxide']),
        'sulphur_dioxide':owm.get('so2',HANOI_AVG['sulphur_dioxide']),'ozone':owm.get('o3',HANOI_AVG['ozone']),
        ...
        'co_lag1':owm.get('co',HANOI_AVG['carbon_monoxide']),'co_lag2':owm.get('co',HANOI_AVG['carbon_monoxide']),
        ...
        'pm2_5_roll3':r3,'pm2_5_roll7':r7,'pm_ratio':pm2_5/(pm10+1),
    }
...
def predict():
    try:
        ...
        if current_pm25 <= 0: return jsonify({'error': 'PM2.5 không hợp lệ'}), 400

        print(f"\n[REQUEST] ESP32 yêu cầu chạy AI, PM2.5={current_pm25}")
        ...
        # Ghi Local Prediction lên Firebase (Có auth)
        requests.put(f"{FIREBASE_URL}/ai_forecast.json?auth={FIREBASE_AUTH}", json={"local": {"aqi": aqi_local, "advice": adv_local}}, timeout=5)
